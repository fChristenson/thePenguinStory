...
public class PenguinService extends IntentService {
    ...
    private void runService() {
        if (penguin == null || penguin.isDead()) {
            Log.d(SERVICE, "penguin is dead, killing service");
            isRunning = false;
            return;
        }
        try {
    ...
    private boolean isTimeToUpdate() {
        ...
        DateTime lastTimeChecked = penguin.getLastTimeChecked();
        DateTime dateTime = time.minus(lastTimeChecked.getMillis());
        int timeLimit = IntegerConstants.TIME_BEFORE_STATUS_UPDATE.get();

        if (dateTime.getMillis() >= timeLimit) {
            penguin.setLastTimeChecked(time.getMillis());
            return true;
        }
        return false;
    }
